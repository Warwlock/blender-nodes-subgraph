...
void Cloth_float(float3 _POS, float3 _PVS, float3 _PWS, float3 _NOS, float3 _NVS, float3 _NWS, float3 _NTS, float3 _TWS, float3 _BTWS, float3 _UV, float3 _SP, float3 _VVS, float3 _VWS, out float4 ColorOut, out float3 Normal)
{
	...
	float _MagicTexture_8090_fac; float4 _MagicTexture_8090_col; node_tex_magic(_POS, 70, 0.5, 5, _MagicTexture_8090_fac, _MagicTexture_8090_col);
	float4 _Bump_8092; node_bump(_POS, -1, 0.3, 0.1, _MagicTexture_8090_col, _NOS, _Bump_8092);
