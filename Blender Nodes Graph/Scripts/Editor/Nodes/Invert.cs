...
namespace BlenderNodesGraph
{
        ...
        public override object GetValue(NodePort port)
        {
            ...
            if (port.fieldName == "Result")
            {
                return sFac_f + sColor_f +
                    "|float4 " + ValueID + " = " +
                    string.Format("node_invert({0}, {1})", sFac, sColor) + ";?" + ValueID;
            }
            else
                return 0f;
        }
    ...
    [CustomNodeEditor(typeof(Invert))]
    public class InvertEditor : NodeEditor
    {
        Invert serializedNode;
        public override void OnBodyGUI()
        {
            if (serializedNode == null) serializedNode = target as Invert;
