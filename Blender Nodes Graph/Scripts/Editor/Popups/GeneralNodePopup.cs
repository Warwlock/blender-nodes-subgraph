...
public class GeneralNodePopup : PopupWindowContent
{
    ...
    void OneTimePosition()
    {
        if (index < 3)
        {
            ...
            {
                if (Event.current.mousePosition.y > Screen.currentResolution.height - height * 1.4f)
                {
                    editorWindow.position = new Rect(GUIUtility.GUIToScreenPoint(
                        new Vector2(Event.current.mousePosition.x, Event.current.mousePosition.y - height)), 
                        new Vector2(0, 0));
                }
                else
                {
