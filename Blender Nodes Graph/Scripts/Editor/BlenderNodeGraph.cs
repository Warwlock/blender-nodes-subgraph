...
namespace BlenderNodesGraph
{
    [HelpURL("https://github.com/Warwlock/blender-nodes-subgraph/wiki")]
    [CreateAssetMenu(fileName = "New Node Graph", menuName = "Blender Nodes Graph", order = 82)]
    public class BlenderNodeGraph : NodeGraph
    {
        ...
        public string graphMenuPath = "Blender Nodes Graph";
        ...
        public void GenerateCode(string savePath = "")
        {
            ...
            includeParagraph = "#include <Assets/Blender Nodes Graph/Scripts/Editor/Includes/Importers.hlsl>\n\n";
        ...
        string GenerateSubgraph(string customFunctionGUID, string customFunctionName, string graphPath = "Blender Nodes Graph", string[] InputSlots = null, string[] OutputSlots = null)
        {
            ...
            for (int i = 0; i < nodes.Count(); i++)
            {
                if (nodes[i].GetType() == typeof(RGBCurves))
                {
                    ...
                    Texture2D curveTexture = new Texture2D(1024, 1024, TextureFormat.RGBA32, false, true);
                    Color color;
                    for (int index = 0; index < 1024; index++)
                    {
                        float t = (index / 1024f) / 16f;

                        for (int fory = 0; fory < 16; fory++)
                        {
                            float x = t + fory * (0.0625f);
                            color = new Color((nodes[i] as RGBCurves).curveR.Evaluate(x),
                                (nodes[i] as RGBCurves).curveG.Evaluate(x),
                                (nodes[i] as RGBCurves).curveB.Evaluate(x),
                                (nodes[i] as RGBCurves).curveC.Evaluate(x));
                            for (int y = fory * 64; y < fory * 64 + 64; y++)
                            {
                                curveTexture.SetPixel(index, y, color);
                            }
                        }
                        //Debug.Log(color);
                    }
    ...
    [CustomNodeGraphEditor(typeof(BlenderNodeGraph))]
    public class BlenderNodeGraphEditor : NodeGraphEditor
    {
        BlenderNodeGraph bng;

        bool option = false;
        public override void OnGUI()
        {
            if (bng == null) bng = target as BlenderNodeGraph;
            if (bng.isEnable)
            {
                bng.isEnable = false;
                RealtimeUpdate();
        ...
        void DrawToolStrip()
        {
            ...
            /*GUILayout.Space(5);
            if (GUILayout.Button("Generate Subgraph - Save as", EditorStyles.toolbarButton))
            {
                OnMenu_ChangePath();
                EditorGUIUtility.ExitGUI();
            }*/
            ...
            /*GUILayout.Space(5);
            GUILayout.Label(new GUIContent("Subgraph Menu Name", "When you click create node in shader graph window, this node graph shows under entered name."));
            bng.graphMenuPath = EditorGUILayout.TextField(bng.graphMenuPath, EditorStyles.toolbarTextField, GUILayout.Width(bng.graphMenuPath.Length * 6.4f + 10));
            */

            GUILayout.FlexibleSpace();
            //GUILayout.Label("Blender Node Graph");
            GUILayout.FlexibleSpace();
