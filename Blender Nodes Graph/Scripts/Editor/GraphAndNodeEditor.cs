    ...
    public class GlobalGraphEditor : Editor {
        public override void OnInspectorGUI() {
            ...
            if (GUILayout.Button("Generate Subgraph", GUILayout.Height(20)))
            {
                ...
                {
                    (graphObj[i] as BlenderNodesGraph.BlenderNodeGraph).newFilePath = Application.dataPath + AssetDatabase.GetAssetPath(graphObj[i]).Substring("Assets".Length);
                    if ((graphObj[i] as BlenderNodesGraph.BlenderNodeGraph).newFilePath != "")
                    {
                        (graphObj[i] as BlenderNodesGraph.BlenderNodeGraph).hlslFileName = System.IO.Path.ChangeExtension(name, ".hlsl");
                        //Debug.Log(hlslFileName);
                        (graphObj[i] as BlenderNodesGraph.BlenderNodeGraph).newFilePath = Application.dataPath + AssetDatabase.GetAssetPath(graphObj[i]).Substring("Assets".Length);
                        (graphObj[i] as BlenderNodesGraph.BlenderNodeGraph).GenerateCode((graphObj[i] as BlenderNodesGraph.BlenderNodeGraph).newFilePath);
                    }
                }
            }
            ...
            if (targets.Length == 1)
            {
                ...
                (graphObj as BlenderNodesGraph.BlenderNodeGraph).graphMenuPath = 
                    EditorGUILayout.TextField((graphObj as BlenderNodesGraph.BlenderNodeGraph).graphMenuPath);
            }
