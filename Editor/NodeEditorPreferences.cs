...
namespace BNGNodeEditor {
    ...
    public static class NodeEditorPreferences {
        ...
        private static BNGNodeEditor.NodeGraphEditor lastEditor;
        ...
        public class Settings : ISerializationCallbackReceiver {
            [SerializeField] private Color32 _gridLineColor = new Color(0.35f, 0.35f, 0.35f);
            ...
            public bool dMode = false;
            public bool useOneColor = false;
            public bool dragToCreate = false;
        ...
        public static Settings GetSettings() {
            if (BNGNodeEditor.NodeEditorWindow.current == null) return new Settings();

            if (lastEditor != BNGNodeEditor.NodeEditorWindow.current.graphEditor) {
                object[] attribs = BNGNodeEditor.NodeEditorWindow.current.graphEditor.GetType().GetCustomAttributes(typeof(BNGNodeEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute), true);
                if (attribs.Length == 1) {
                    BNGNodeEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute attrib = attribs[0] as BNGNodeEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute;
                    lastEditor = BNGNodeEditor.NodeEditorWindow.current.graphEditor;
        ...
        public static SettingsProvider CreateXNodeSettingsProvider() {
            SettingsProvider provider = new SettingsProvider("Preferences/Blender Nodes Editor", SettingsScope.User) {
                guiHandler = (searchContext) => { BNGNodeEditor.NodeEditorPreferences.PreferencesGUI(); },
                keywords = new HashSet<string>(new [] { "blender", "node", "editor", "graph", "connections", "noodles", "ports" })
            };
            return provider;
        }
#endif
        ...
        private static void PreferencesGUI() {
            ...
            if (GUILayout.Button(new GUIContent("Documentation", ""), GUILayout.Width(200), GUILayout.Height(20))) Application.OpenURL("https://github.com/Warwlock/blender-nodes-subgraph/wiki");
            GUILayout.FlexibleSpace();
        ...
        private static void SystemSettingsGUI(string key, Settings settings) {
            ...
            settings.dMode = EditorGUILayout.Toggle(new GUIContent("Enable Debug Mode", "Debug mode for development"), settings.dMode);
        ...
        private static void NodeSettingsGUI(string key, Settings settings) {
            ...
            settings.useOneColor = EditorGUILayout.Toggle(new GUIContent("Use One Color", "Shows only one color for ports if selected"), settings.useOneColor);
            if (GUI.changed) {
                SavePrefs(key, settings);
                NodeEditorWindow.RepaintAll();
            }
