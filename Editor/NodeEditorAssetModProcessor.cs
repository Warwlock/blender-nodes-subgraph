...
namespace BNGNodeEditor {
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (BNGNode.Node) && !scriptType.IsSubclassOf (typeof (BNGNode.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    BNGNode.Node node = objs[k] as BNGNode.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (BNGNode.NodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                BNGNode.NodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (BNGNode.NodeGraph)) as BNGNode.NodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as BNGNode.Node)) graph.nodes.Add(objs[u] as BNGNode.Node);
                }
            }
        }
    }
