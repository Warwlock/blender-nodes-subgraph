...
namespace BNGNodeEditor {
    ...
    /// renaming a <see cref="BNGNode.NodeGraph"/> asset, it appears that sometimes the v2 AssetDatabase will swap which asset
    /// is the main asset (present at top level) between the <see cref="BNGNode.NodeGraph"/> and one of its <see cref="BNGNode.Node"/>
    /// sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    /// finds a case where a <see cref="BNGNode.Node"/> has been made the main asset it will swap it back to being a sub-asset
