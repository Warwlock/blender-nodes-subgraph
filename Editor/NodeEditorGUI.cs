...
namespace BNGNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        private List<BNGNode.Node> culledNodes;
        ...
        void ShowPortContextMenu(BNGNode.NodePort hoveredPort) {
            GenericMenu contextMenu = new GenericMenu();
            ...
            foreach (BNGNode.Node node in graph.nodes) {
                ...
                foreach (BNGNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        BNGNode.NodePort input = output.GetConnection(k);
        ...
        private void DrawNodes() {
            ...
            if (Selection.activeObject != null && Selection.activeObject is BNGNode.Node) {
            ...
            List<BNGNode.NodePort> removeEntries = new List<BNGNode.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<BNGNode.Node>();
            for (int n = 0; n < graph.nodes.Count; n++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                BNGNode.Node node = graph.nodes[n];
                ...
                } else if (culledNodes != null) if (culledNodes.Contains(node)) continue;
                ...
                Color nodeBackground = new Color(0.5f, 0.5f, 0.5f);
                if (node.useCustomColor)
                {
                    nodeBackground = node.nodeColor;
                }

                GUILayout.BeginArea(new Rect(nodePos, new Vector2(nodeEditor.GetWidth(), 4000)));

                bool selected = selectionCache != null ? selectionCache.Contains(graph.nodes[n]) : false;
                ...
                if (selected)
                {
                    ...
                    GUI.color = nodeBackground;
                    GUILayout.BeginVertical(style);

                    GUI.color = nodeEditor.GetTint();
                    Texture2D tex = GUI.skin.box.normal.background;
                    GUI.skin.box.normal.background = NodeEditorResources.nodeBody;
                    GUI.skin.box.border = new RectOffset(16, 16, 30, 24);
                    GUI.Box(new Rect(0, 0, nodeEditor.GetWidth(), 30), GUIContent.none);
        ...
        private bool ShouldBeCulled(BNGNode.Node node) {
