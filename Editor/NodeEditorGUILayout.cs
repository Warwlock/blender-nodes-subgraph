...
namespace BNGNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            BNGNode.Node node = property.serializedObject.targetObject as BNGNode.Node;
            BNGNode.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, BNGNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, BNGNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options)
        {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                if (port.direction == BNGNode.NodePort.IO.Input) {
                    // Get data from [Input] attribute
                    BNGNode.Node.ShowBackingValue showBacking = BNGNode.Node.ShowBackingValue.Unconnected;
                    BNGNode.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == BNGNode.Node.ShowBackingValue.Never ||
                        (showBacking == BNGNode.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    if (dynamicPortList) {
                        ...
                        BNGNode.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : BNGNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                        case BNGNode.Node.ShowBackingValue.Unconnected:
                            ...
                            break;
                        case BNGNode.Node.ShowBackingValue.Never:
                            // Display a label
                            EditorGUILayout.LabelField(label != null ? label : new GUIContent(property.displayName));
                            break;
                        case BNGNode.Node.ShowBackingValue.Always:
                ...
                } else if (port.direction == BNGNode.NodePort.IO.Output) {
                    // Get data from [Output] attribute
                    BNGNode.Node.ShowBackingValue showBacking = BNGNode.Node.ShowBackingValue.Unconnected;
                    BNGNode.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    if (dynamicPortList) {
                        Type type = GetType(property);
                        BNGNode.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : BNGNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                    ...
                    rect = GUILayoutUtility.GetLastRect();
                    rect.position = rect.position + new Vector2(rect.width + 2, spacePadding + 2);
                }

                rect.size = new Vector2(13, 13);
        ...
        public static void PortField(BNGNode.NodePort port, params GUILayoutOption[] options) {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, BNGNode.NodePort port, params GUILayoutOption[] options) {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, BNGNode.NodePort port) {
            if (port == null) return;

            Rect rect = new Rect(position, new Vector2(13, 13));
        ...
        public static void AddPortField(BNGNode.NodePort port) {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(BNGNode.NodePort input, BNGNode.NodePort output) {
...
#region Obsolete
        ...
        public static bool IsInstancePortListPort(BNGNode.NodePort port) {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, BNGNode.NodePort.IO io, BNGNode.Node.ConnectionType connectionType = BNGNode.Node.ConnectionType.Multiple, BNGNode.Node.TypeConstraint typeConstraint = BNGNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
        ...
        public static bool IsDynamicPortListPort(BNGNode.NodePort port) {
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, BNGNode.NodePort.IO io, BNGNode.Node.ConnectionType connectionType = BNGNode.Node.ConnectionType.Multiple, BNGNode.Node.TypeConstraint typeConstraint = BNGNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            BNGNode.Node node = serializedObject.targetObject as BNGNode.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x => {
                ...
                return new { index = -1, port = (BNGNode.NodePort) null };
            }).Where(x => x.port != null);
            List<BNGNode.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<BNGNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, BNGNode.NodePort.IO io, BNGNode.Node.ConnectionType connectionType, BNGNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    BNGNode.NodePort port = node.GetPort(fieldName + " " + index);
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            BNGNode.NodePort port = node.GetPort(fieldName + " " + i);
                            BNGNode.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    if (io == BNGNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, BNGNode.Node.TypeConstraint.None, newName);
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++) {
                            for (int j = 0; j < dynamicPorts[k].ConnectionCount; j++) {
                                BNGNode.NodePort other = dynamicPorts[k].GetConnection(j);
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    if (io == BNGNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
